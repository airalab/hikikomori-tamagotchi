...
esp_err_t remove_nvs_key(const char* key) {
    nvs_handle_t handle;
    ESP_LOGI(TAG, "Deleting %s from flash", key);
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error (%s) erasing key!", esp_err_to_name(err));
    } else {
        ESP_LOGI(TAG, "Key deleted successfully");
    }

    // Commit changes to ensure the key is deleted
    ...
    // Return ESP_OK only if everything succeeded
