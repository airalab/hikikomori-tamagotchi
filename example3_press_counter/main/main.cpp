...
static void send_datalog_counter() {
    ...
    set_sending_datalog_screen(message);
    buttonPressCount = 0;
    ...
    if ( WiFi.status() != WL_CONNECTED ) {
        ...
        remove_nvs_key(WIFI_SSID_NVS_KEY);
        esp_restart();
    }
    ESP_LOGI(TAG, "WiFi connected");
    robonomics.setup(user_data.host_url.c_str());
    const char* res = robonomics.sendDatalogRecord(message);
...
void get_or_generate_private_key() {
    ...
    ESP_LOGI(TAG, "Robonomics Private Key: %s", robonomics.getPrivateKey());
}
