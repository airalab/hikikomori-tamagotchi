...
void set_display_brightness(esp_lcd_panel_t *panel, uint8_t brightness) {
    // The brightness value should be between 0 and 255
    if (brightness > 255) {
        brightness = 255;
    }
    st7789t_panel_t *st7789t = __containerof(panel, st7789t_panel_t, base);
    ...
    // Send the command and the brightness value
    int err = esp_lcd_panel_io_tx_param(io, LCD_CMD_WRDISBV, &brightness, 1);
    if (err != 1) {
        ESP_LOGE("LCD", "Failed to set brightness: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI("LCD", "Brightness set to %d", brightness);
    }
}